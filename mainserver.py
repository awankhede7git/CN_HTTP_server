...
class server():
    ...
    def get_ext_or_type(self, ext_type,indication):
        ...
        file_type = {'.txt':'text/plain', '.html':'text/html', '.png':'image/png','.jpg':'image/jpg', '.ico': 'image/x-icon',  '.gif': 'image/gif', '.php':'application/x-www-form-urlencoded', '': 'text/plain', '.jpeg':'image/webp', '.pdf': 'application/pdf', '.js': 'application/javascript', '.css': 'text/css', '.mp3' : 'audio/mpeg', '.mp4': 'video/mp4'}

        file_ext = {v: k for k, v in file_type.items()}
        ...
        if indication == "type":
            return file_type.get(ext_type, '')

        elif indication == "ext":
            return file_ext.get(ext_type, '')
    ...
    def post_req(self,h_lines,file_loc,client,file_content,byte_format,address,request_line):
        ...
        if h_lines['Content-Length'] == '': 
            self.status_code(client,411,request_line,address)
            return
    ...
    def put_req(self,h_lines,file_loc,client,file_content,byte_format,address,request_line):
        ...
        while len(file_content) < int(h_lines['Content-Length']):
            if not byte_format :
                ...
                file_content += client.recv(self.size)
        # file_content = file_content.encode('utf-8') if not byte_format else file_content
    ...
    def client_request(self,client_socket,address):
        ...
        while True:
            ...
            try :
                ...
                m = message[1].decode()   #body in message[1]
                #print("MESSAGE: ",m)
                message[1] = m
            except :
...
def main():
    ...
    try:
        serversocket.bind(('', port))  # Bind to all available interfaces
    ...
    print(f"Connect server on ({IP}, {port})")
    print(f"Url: http://{IP}:{port}  The file name")
